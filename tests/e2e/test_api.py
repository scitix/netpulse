...
def test_job_list_empty_when_no_jobs(api_server):
    ...
    resp = requests.get(f"{API_BASE}/job", headers=_headers(), timeout=5)
    assert resp.status_code == 200
    body = resp.json()
    assert body["code"] == 200
    assert body["data"] == [] or body["data"] is None


def test_worker_list_reports_fifo_worker(fifo_worker, api_server):
    """FIFO worker process should be visible via /worker listing."""
    fifo_queue = utils.g_config.get_fifo_queue_name()
    resp = requests.get(
        f"{API_BASE}/worker", params={"queue": fifo_queue}, headers=_headers(), timeout=5
    )
    assert resp.status_code == 200
    workers = resp.json().get("data") or []
    assert any(fifo_queue in (w.get("queues") or []) for w in workers)


def test_worker_list_reports_node_worker(node_worker, api_server):
    """Node worker should register its queue and appear in /worker listing."""
    node_queue = utils.g_config.get_node_queue_name(socket.gethostname())
    resp = requests.get(
        f"{API_BASE}/worker", params={"queue": node_queue}, headers=_headers(), timeout=5
    )
    assert resp.status_code == 200
    workers = resp.json().get("data") or []
