    ...
    if isinstance(body, dict):
        ...
        if isinstance(result, str):
            ...
            if "type" in parsed_result and "message" in parsed_result:
                ...
                logger.info(f"    🏷️  Error Type: {parsed_result['type']}")
            ...
            if result.get("error"):
                logger.info("    ❌ Job Execution Failed")
                error = result["error"]
                logger.info(f"    🏷️  Error Type: {error.get('type', 'Unknown')}")
                logger.info("    📝 Error Details:")
...
async def clear_messages():
    ...
    logger.info(f"🗑️  Cleared {old_count} webhook message records")
    return {
        "status": "cleared",
...
if __name__ == "__main__":
    ...
    print("🗑️  Clear Records: DELETE http://localhost:8888/webhook/clear")
    print("=" * 60)
