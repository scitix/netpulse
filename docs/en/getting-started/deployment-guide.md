...
- Initialize Vault (if needed), generate `unseal_key` and `root_token`, and update `.env` file
- Start all Docker services (Vault starts first, then other services after Vault is initialized)
- Verify deployment
- Display connection information

!!! note "Vault Automatic Initialization"
    The deployment script handles Vault initialization automatically:
    1. Start Vault service first
    2. Initialize Vault (if needed), generate `unseal_key` and `root_token`
    3. Update `.env` file with `VAULT_UNSEAL_KEY` and `VAULT_TOKEN`
    4. Unseal Vault and enable KV v2 secret engine
    5. Start other services, which will now use the correct `VAULT_TOKEN`
...
# These are automatically generated during Docker deployment
...
!!! note "Vault Configuration Notes"
    - `VAULT_UNSEAL_KEY` and `VAULT_TOKEN` are automatically generated by `docker_auto_deploy.sh`
    - For manual setup, you need to initialize Vault manually and set these values
    - The script handles Vault initialization, unsealing, and data persistence automatically
