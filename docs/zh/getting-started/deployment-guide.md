...
!!! note "Vault 自动初始化"
    脚本会按以下顺序启动服务：
    1. **先启动 Vault**：确保 Vault 容器运行
    2. **初始化 Vault**（如需要）：首次部署时自动初始化，生成 `unseal_key` 和 `root_token`
    3. **更新 .env 文件**：将 Vault 密钥自动写入 `.env` 文件
    4. **启动其他服务**：此时 `.env` 已包含正确的 Vault token，容器启动时直接使用
    
    这样确保所有容器启动时都使用正确的 Vault 认证信息，无需重启。
...
# Vault Configuration (Auto-generated by script)
# 首次部署时，脚本会自动初始化 Vault 并生成以下变量：
# VAULT_UNSEAL_KEY=xxx              # Vault 解封密钥（自动生成）
# VAULT_TOKEN=xxx                   # Vault 根令牌（自动生成）
...
!!! note "Vault 配置说明"
    - **首次部署**：脚本会自动初始化 Vault，生成 `VAULT_UNSEAL_KEY` 和 `VAULT_TOKEN` 并写入 `.env`
    - **后续部署**：脚本会从 `.env` 读取 Vault 密钥，自动解封 Vault（如需要）
    - **数据持久化**：Vault 数据存储在 Docker volume 中，容器重启不会丢失
