...
NetPulse 通过其插件系统提供可扩展的驱动支持。用户可以使用内置支持的四种驱动，也可以根据需要开发自定义驱动。
...
| Paramiko  | SSH            | Linux 服务器             | 原生 SSH、文件传输、代理连接、sudo   | paramiko~=3.0.0    |
...
Paramiko 驱动是 NetPulse 中用于管理 Linux 服务器的驱动，基于原生 SSH 协议实现。Paramiko 驱动支持命令执行、文件传输、代理连接、sudo 等高级功能，适用于系统监控、配置管理、软件部署等场景。

Paramiko 驱动默认使用 FIFO 队列策略（短连接），适合长时间运行任务和文件传输等场景。与 Netmiko 驱动不同，Paramiko 驱动不实现长连接复用，每次任务执行后都会断开连接。
