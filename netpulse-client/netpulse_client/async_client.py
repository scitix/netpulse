...
class AsyncNetPulseClient:
    ...
    async def execute(
        ...
        job_id = await self._submit_job(
            self._build_url("/device/execute"), request_data.model_dump(exclude_none=True)
        )
    ...
    async def bulk(
        ...
        # 提交任务
        job_id = await self._submit_job(
            self._build_url("/device/bulk"), request_data.model_dump(exclude_none=True)
        )
    ...
    async def get_job_status(self, job_id: str) -> Optional[JobInfo]:
        """获取任务状态"""
        try:
            data = await self._make_request("GET", self._build_url("/job"), params={"id": job_id})
    ...
    async def wait_for_job(
        self, handle: AsyncJobHandle, poll_interval: float = 0.4
                ...
                if job_info.status in ["finished", "failed"]:
                    ...
                    if handle.job_id in self._job_callbacks:
                        for callback in self._job_callbacks[handle.job_id]:
                            try:
                                await callback(job_info) if asyncio.iscoroutinefunction(
                                    callback
                                ) else callback(job_info)
                            except Exception as e:
                    ...
                    if handle.task_type == "command":
                        return CommandResult(
                            ...
                            device_host=handle.device_hosts[0]
                            if handle.device_hosts
                            else "unknown",
    ...
    async def cancel_job(self, job_id: str) -> bool:
        ...
        try:
            data = await self._make_request(
                "DELETE", self._build_url("/job"), params={"id": job_id}
            )
    ...
    async def get_workers(
        ...
        try:
            data = await self._make_request("GET", self._build_url("/worker"), params=params)
        ...
        while total_elapsed < max_total_time:
            try:
                result = await self._make_request(
                    "GET", self._build_url("/job"), params={"id": job_id}
                )
    ...
    async def atest_connection(
        ...
        result = await self._make_request(
            "POST", self._build_url("/device/test-connection"), json=data
        )
        if result.get("code") == 0 and result.get("data"):
    ...
    async def arender_template(
        self, name: str, template: str, context: Optional[Dict] = None
    ) -> Dict:
        """异步渲染模板"""
    ...
    async def aparse_template(
        self, name: str, template: str, context: Optional[str] = None
    ) -> Dict:
        """异步解析模板"""
...
# 工厂函数
async def create_async_client(endpoint: str, api_key: str, **kwargs) -> AsyncNetPulseClient:
    """创建异步客户端实例"""
